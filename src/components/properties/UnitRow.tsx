...
export function UnitRow({ unit }: { unit: Unit }) {
  ...
  return (
    <>
      <div
        className="flex items-center gap-3 px-4 py-3.5 border rounded-xl hover:border-primary transition-colors cursor-pointer"
        onClick={openEdit}
      >
        <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${unit.is_occupied ? 'bg-green-500' : 'bg-slate-300'}`} />
        <div className="w-28 font-bold text-[14px]">{unit.name}</div>
        <div className="flex-1 text-sm text-muted-foreground">{unit.is_occupied ? 'Occupied' : 'Vacant'}</div>
        ...
        <div className="w-24 text-right font-bold text-sm">{rm(unit.price)}/mo</div>
        <DropdownMenu>
          <DropdownMenuTrigger
